...
class CtdetDetector(BaseDetector):
  ...
  def show_results(self, debugger, image, results, image_name, header=None):
      ...
      for bbox in results[j]:
        if bbox[4] > self.opt.vis_thresh:
          if not self.bbox_valid(bbox, imsize, dis):
            ...
            if min_dis and not self.distance_valid(result_new,bbox,min_dis):
              continue
            else:
              result_new.append(bbox)
      results[j]=result_new
    ...
    if self.opt.visual == True and self.opt.mode!='fiber':
      debugger.save_all_imgs(path=img_path, genID=True)
    if self.opt.data_type == 'mrc':
  ...
  def distance_valid(self,result_new,bbox,min_dis=25):
    for i in range(len(result_new)):
      d=result_new[i]
      if(np.sqrt((d[0]-bbox[0])**2+(d[1]-bbox[1])**2)<min_dis):
        if d[4]<bbox[4]:
          result_new[i]=bbox
        return False
    return True
